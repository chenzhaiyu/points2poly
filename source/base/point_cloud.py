...
def get_patch_kdtree(
    ...
    if patch_radius <= 0.0:
        pts_dists_ms, patch_pts_ids = kdtree.query(x=query_point, k=points_per_patch, workers=n_jobs)
    else:
        patch_pts_ids = kdtree.query_ball_point(x=query_point, r=patch_radius, workers=n_jobs)
